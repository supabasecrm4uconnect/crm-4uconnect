...
- O primeiro acesso ao Preview revelou que `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` não estavam disponíveis nesse ambiente (`supabaseUrl is required`). O bootstrap agora detecta essa situação antes de importar o cliente Supabase e exibe uma tela de configuração pendente sem revelar valores. O HTML também carregava seis famílias do Google Fonts bloqueadas pela CSP; essa dependência externa desnecessária foi removida e o título que usava Sora passou a herdar a Inter já incorporada ao bundle.
- O aviso do `vercel.live` é da barra de feedback injetada pela Vercel e permanece bloqueado pela CSP de scripts. Ele não é dependência do CRM e não deve motivar a liberação global de scripts externos.
...
- Após validação de login e gravação de leads no Preview, somente `crm-web/` foi promovido à `main` no commit `e6905a9`. A extensão e o banco permaneceram byte a byte iguais à produção anterior; a extensão já publicada continua em uso e compatível pelo domínio `crm-4uconnect.vercel.app`.
- O deploy de produção da Vercel concluiu com sucesso e os domínios `crm-4uconnect.vercel.app` e `connect-crm.vercel.app` responderam HTTP 200 servindo o bundle `index-CAUXCQ8X.js`.
